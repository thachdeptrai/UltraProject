...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    private int lY;
    // private static readonly string txtIP = Assets.Script.Functions.String_EnFunctions._Instance.Hex2String(Assets.Script.Functions.String_EnFunctions.getInstance().ReverseString(GameTCP.readFileText("Unnamed")), null);
    private static readonly string txtIP = "Nro:localhost:14445:0,0,0";
    ...
    public static string linkweb = "http://ngocthack.bio.link/";
    ...
    public override void paint(mGraphics g)
    {
        ...
        int num2 = 2;
        //string title = "Độc Quyền Bởi " + String_EnFunctions._Instance.Hex2String(String_EnFunctions._Instance.ReverseString("46 F6 D4 27 96 16 84"), null);
        mFont.tahoma_7_white.drawString(g, "v" + GameMidlet.VERSION + "[" + mGraphics.zoomLevel + "]", GameCanvas.w - 2, num2 + 15, 1, mFont.tahoma_7_grey);
        //mFont.tahoma_7b_white.drawStringBd(g, title, GameCanvas.w / 2, 0, 3, mFont.tahoma_7_grey);
        //string title2 = "Ngọc Rồng KimKan";
        //mFont.tahoma_7_white.drawStringBd(g, title2, GameCanvas.w / 2, 10, 3, mFont.tahoma_7_grey);
        //mFont.bigNumber_While.drawString(g, title3, GameCanvas.w / 2 + 20, 10, 3); ;
        if (!isGetData || loadScreen)
        {
            //ManagerCommand.paint(g);
            if (mSystem.clientType == 1 && !GameCanvas.isTouch)
            {
                ...
                {
                    index++;
                    index = (byte)(index % (linkweb.Length + 1));
                    if (index > linkweb.Length)
                    {
                        index = 0;
                    }
                }
                mFont.tahoma_7_white.drawStringBd(g, linkweb.Substring(0, index), GameCanvas.w - 2, GameCanvas.h - 15, 1, mFont.tahoma_7_grey);
            ...
            else
            {
                ...
                mFont.tahoma_7_white.drawStringBd(g, linkweb.Substring(0, index), GameCanvas.w - 2, GameCanvas.h - 295, 1, mFont.tahoma_7_grey);
            }
        }
        else
        {
            ...
            mFont.tahoma_7_white.drawStringBd(g, linkweb.Substring(0, index), GameCanvas.w - 2, GameCanvas.h - 240, 1, mFont.tahoma_7_grey);
        }
        ...
        if (GameCanvas.currentDialog == null)
        {
            if (!loadScreen)
            {
                if (!bigOk)
                {
                    g.drawImage(LoginScr.imgTitle, GameCanvas.hw, GameCanvas.hh - 15, 3);
                    if (!isGetData)
                    {
            ...
            else
            {
                ...
                {
                    g.drawImage(LoginScr.imgTitle, GameCanvas.hw, num4 + 10, 3);
                }
                for (int i = 0; i < cmd.Length; i++)
                {
                    cmd[i].paint(g);
                ...
                if (testConnect == -1)
                {
                    ...
                    {
                        g.drawRegion(GameScr.imgRoomStat, 0, 14, 7, 7, 0, (GameCanvas.w - mFont.tahoma_7b_white.getWidth(cmd[2 + nCmdPlay].caption) >> 1) - 10, cmd[2 + nCmdPlay].y + 10, 0);
                    }
                }
                else
                {
                    g.drawRegion(GameScr.imgRoomStat, 0, testConnect * 7, 7, 7, 0, (GameCanvas.w - mFont.tahoma_7b_white.getWidth(cmd[2 + nCmdPlay].caption) >> 1) - 10, cmd[2 + nCmdPlay].y + 9, 0);
                }
    ...
    public void perform(int idAction, object p)
    {
        ...
        if (idAction == 8)
        {
            ...
            myVector2.addElement(new Command("Auto Manager", this, 999, null));
