...
public class Panel : IActionListener, IChatable
{
		...
		for (int i = 0; i < currentListLength; i++)
		{
			...
			if (i == currentListLength - 1)
			{
				...
				if (!isLock)
				{
					if (!isFriendLock)
					{
						global::mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.not_lock_trade, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
					}
					else
					{
						global::mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.locked_trade, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
					}
				}
				else if (isFriendLock)
				{
					...
					((i != selected) ? global::mFont.tahoma_7b_dark : global::mFont.tahoma_7b_green2).drawString(g, mResources.done, xScroll + wScroll - 22, num2 + 7, 2);
					global::mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.locked_trade, xScroll + 5, num2 + num4 / 2 - 4, global::mFont.LEFT);
				}
				else
				{
					global::mFont.tahoma_7_grey.drawString(g, mResources.opponent + mResources.not_lock_trade, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
			...
			if (i == currentListLength - 2)
			{
				if (isMe)
				{
					...
					if (!isAccept)
					{
						...
						{
							g.drawImage((i != selected) ? GameScr.imgLbtn2 : GameScr.imgLbtnFocus2, xScroll + wScroll - 5, num2 + 2, StaticObj.TOP_RIGHT);
							((i != selected) ? global::mFont.tahoma_7b_dark : global::mFont.tahoma_7b_green2).drawString(g, mResources.mlock, xScroll + wScroll - 22, num2 + 7, 2);
							global::mFont.tahoma_7_grey.drawString(g, mResources.you + mResources.not_lock_trade, xScroll + 5, num2 + num4 / 2 - 4, global::mFont.LEFT);
						}
						else
						{
							g.drawImage((i != selected) ? GameScr.imgLbtn2 : GameScr.imgLbtnFocus2, xScroll + wScroll - 5, num2 + 2, StaticObj.TOP_RIGHT);
							((i != selected) ? global::mFont.tahoma_7b_dark : global::mFont.tahoma_7b_green2).drawString(g, mResources.CANCEL, xScroll + wScroll - 22, num2 + 7, 2);
							global::mFont.tahoma_7_grey.drawString(g, mResources.you + mResources.locked_trade, xScroll + 5, num2 + num4 / 2 - 4, global::mFont.LEFT);
						}
					}
				}
				else if (!isFriendLock)
				{
					global::mFont.tahoma_7b_dark.drawString(g, mResources.not_lock_trade_upper, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
				}
				else
				{
					global::mFont.tahoma_7b_dark.drawString(g, mResources.locked_trade_upper, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
				}
				continue;
			}
			if (i == currentListLength - 3)
			{
				...
				global::mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys((!isMe) ? friendMoneyGD : moneyGD) + " " + mResources.XU, num + 5, num2 + 11, 0);
				global::mFont.tahoma_7_green.drawString(g, mResources.money_trade, num + 5, num2, 0);
				continue;
			}
			if (myVector.size() == 0)
			{
	...
	private void paintMapTrans(mGraphics g)
	{
		...
		for (int i = 0; i < mapNames.Length; i++)
		{
			...
			if (num2 - cmy <= yScroll + hScroll && num2 - cmy >= yScroll - ITEM_HEIGHT)
			{
				...
				mFont.tahoma_7_white.drawStringBd(g, planetNames[i], 5, num2 + 11, 0,mFont.tahoma_7_grey);
			}
		}
		paintScrollArrow(g);
	}
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				if (i == 0)
				{
					for (int j = 0; j < num; j++)
					{
						...
						global::mFont.tahoma_7_grey.drawString(g, string.Empty + j, xScroll + j * TAB_W_NEW + TAB_W_NEW / 2, yScroll + 11 + num9, global::mFont.CENTER);
					}
					continue;
				}
				g.setColor((i != selected) ? 15196114 : 16383818);
		...
		else
		{
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				if (!archivement.isFinish)
				{
					...
					mFont.tahoma_7_green.drawString(g, archivement.money + " " + archivement.textMoney, num + num3 - 5, num2, mFont.RIGHT);
					SmallImage.drawSmallImage(g, archivement.icon, num + num3 - 15, num2 + 11, 0, mFont.RIGHT);
					mFont.tahoma_7_red.drawString(g, archivement.info2, num + 5, num2 + 11, 0);
				}
				else if (archivement.isFinish && !archivement.isRecieve)
				{
					mFont.tahoma_7.drawString(g, archivement.info1, num + 5, num2, 0);
					mFont.tahoma_7_blue.drawString(g, mResources.reward_mission + archivement.money + " " + archivement.textMoney, num + 5, num2 + 11, 0);
					SmallImage.drawSmallImage(g, archivement.icon, num + num3 - 15, num2 + 11, 0, mFont.RIGHT);
					if (i == selected)
					{
			...
			if (j == vItemCombine.size())
			{
				if (vItemCombine.size() > 0)
				{
					...
					if ((j == selected && keyTouchCombine == 1) || (!GameCanvas.isTouch && j == selected))
					{
						...
						global::mFont.tahoma_7b_green2.drawString(g, mResources.UPGRADE, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
					}
					else
					{
						g.drawImage(GameScr.imgLbtn, xScroll + wScroll / 2, num2 + num4 / 2 + 1, StaticObj.VCENTER_HCENTER);
						global::mFont.tahoma_7b_dark.drawString(g, mResources.UPGRADE, xScroll + wScroll / 2, num2 + num4 / 2 - 4, global::mFont.CENTER);
					}
				}
				continue;
			}
	...
	{
		mFont.tahoma_7b_white.drawString(g, "Auto Ultra" + " " + "2.4.0", 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		...
		mFont.tahoma_7_yellow.drawString(g,"Chưa Đăng Kí", 60, 39, mFont.LEFT, mFont.tahoma_7_grey);
	}
