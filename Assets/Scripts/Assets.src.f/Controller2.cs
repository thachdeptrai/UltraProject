				...
				case 31:
				{
					...
					sbyte b12 = msg.reader().readByte();
							 if (b12 == 2)
                            {
								...
								if (type == 0)
                                {
									var count = msg.reader().readInt();
									if (num15 == Char.myCharz().charID)
                                    {
										Char.myCharz().Roles.Clear();
										for (int i = 0; i < count; i++)
                                        {
											var role = new Role();
											role.Small = msg.reader().readShort();
											role.Frame = msg.reader().readInt();
											role.Dx = msg.reader().readShort();
											role.Dy = msg.reader().readShort();
											Char.myCharz().Roles.Add(role);
                                        }
                                    }
                                    else {
										var characer = GameScr.findCharInMap(num15);
										if (characer == null) return;
										characer.Roles.Clear();
										for (int i = 0; i < count; i++)
										{
											var role = new Role() { Small = msg.reader().readShort(), Frame = msg.reader().readInt(), Dx = msg.reader().readShort(), Dy = msg.reader().readShort() };

											characer.Roles.Add(role);
										}
									}
                                }
                                else
